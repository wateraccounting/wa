...
def DownloadData(Dir, Startdate, Enddate, latlim, lonlim, cores):
    ...
    if IsHorTilesNeeded != 0 or IsVerTilesNeeded != 0:
        for Date in Dates:
            ...
            # Define the size of the total GPP picture									
            ...
            for YnameChunk in range(int(VerticalTiles), 0, -1):
                ...
                for XnameChunk in range(1, int(HorizontalTiles)+1):
                    ...
                    #  Define GPP chunk file name, Open this file and open the array  								
                    file_name=os.path.join(output_folder, 'GPP_MOD17_kg-C-m^-2_8-daily_'+Date.strftime('%Y')+'.' + Date.strftime('%m')+'.' + Date.strftime('%d')+'_chunk_h' + str(XnameChunk) + 'v'+ str(YnameChunk) + '.tif')
            ...
            Save_as_Gtiff(TotData, GPPfileName, lonlim, latlim)         
...
def RetrieveData(Date, args):
    ...
    if IsHorTilesNeeded==0 and IsVerTilesNeeded==0:
        GPPfileName = os.path.join(output_folder, 'GPP_MOD17_kg-C-m^-2_8-daily_' + Date.strftime('%Y') + '.' + Date.strftime('%m') + '.' + Date.strftime('%d') + '.tif')
        Save_as_Gtiff(data,GPPfileName,lonlim1,latlim1)                     
                    
    else:
        GPPfileName = os.path.join(output_folder, 'GPP_MOD17_kg-C-m^-2_8-daily_' + Date.strftime('%Y') + '.' + Date.strftime('%m') + '.' + Date.strftime('%d') + '_chunk_h' + str(lonname) + 'v' + str(latname) + '.tif')
        Save_as_Gtiff(data,GPPfileName,lonlim1,latlim1) 
    return True
...
def Collect_data(TilesHorizontal,TilesVertical,Date,output_folder):
    ...
    for Vertical in range(int(TilesVertical[0]), int(TilesVertical[1])+1):
        ...
        for Horizontal in range(int(TilesHorizontal[0]), int(TilesHorizontal[1]) + 1):
            ...
            url = 'http://e4ftl01.cr.usgs.gov/MOLT/MOD17A2H.006/' + Date.strftime('%Y') + '.' + Date.strftime('%m') + '.' + Date.strftime('%d') + '/' 
