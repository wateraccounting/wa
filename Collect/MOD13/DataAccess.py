...
def DownloadData(Dir, Startdate, Enddate, latlim, lonlim, cores):
    ...
    if IsHorTilesNeeded != 0 or IsVerTilesNeeded != 0:
        for Date in Dates:
            ...
            TotSizeY = int((latlim[1] - latlim[0]) * 400)
            TotSizeX = int((lonlim[1] - lonlim[0]) * 400)
            ...
            for YnameChunk in range(int(VerticalTiles), 0, -1):
                ...
                for XnameChunk in range(1, int(HorizontalTiles)+1):
                    ...
                    file_name = os.path.join(output_folder, 'NDVI_MOD13Q1_-_16-daily_'+Date.strftime('%Y')+'.' + Date.strftime('%m')+'.' + Date.strftime('%d')+'_chunk_h' + str(XnameChunk) + 'v'+ str(YnameChunk) + '.tif')
...
def RetrieveData(Date, args):
    ...
    if IsHorTilesNeeded == 0 and IsVerTilesNeeded == 0:
        ...
        NDVIfileName = os.path.join(output_folder, 'NDVI_MOD13Q1_-_16-daily_' + Date.strftime('%Y') + '.' + Date.strftime('%m') + '.' + Date.strftime('%d') + '_chunk_h' + str(lonname) + 'v' + str(latname) + '.tif')
        Save_as_Gtiff(data, NDVIfileName, lonlim1, latlim1) 
...
def Collect_data(TilesHorizontal,TilesVertical,Date,output_folder):
    ...
    for Vertical in range(int(TilesVertical[0]), int(TilesVertical[1])+1):
        ...
        for Horizontal in range(int(TilesHorizontal[0]), int(TilesHorizontal[1]) + 1):
            ...
            url = 'http://e4ftl01.cr.usgs.gov/MOLT/MOD13Q1.006/' + Date.strftime('%Y') + '.' + Date.strftime('%m') + '.' + Date.strftime('%d') + '/' 
