...
Description:
This script collects SSEBop data from the UNESCO-IHE FTP server. The data has a
...
def DownloadData(Dir, Startdate, Enddate, latlim, lonlim, Waitbar):
    ...
    for Date in Dates:
        ...
        Filename_out= os.path.join(output_folder,'ETa_SSEBop_mm-month-1_monthly_%s.%02s.%02s.tif' %(Date.strftime('%Y'), Date.strftime('%m'), Date.strftime('%d')))
        
        # Define end filename
        Filename_in = os.path.join("%s" %year, "m%s%02d.tif" %(str(year)[2:], month))
        ...
        if not os.path.exists(Filename_out):
            try:
                Download_SSEBop_from_WA_FTP(local_filename, Filename_in)
...
def Download_SSEBop_from_WA_FTP(local_filename, Filename_in):           
    """
    ...
    Filename_in -- name of the end file with the monthly SSEBop data
    """      
