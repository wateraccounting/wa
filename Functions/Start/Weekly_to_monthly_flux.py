    ...
    for date in Dates:
        ...
        for EightDays in DOYs_oneMonth[:,0]:
            ...
            if EightDays == DOYs_oneMonth[:,0][0]:
                Weight =  Weight * int(DOYs_oneMonth[:,1][0] + 7 - int(DOY_month_start))
        ...
        # Define output name
        output_name = os.path.join(Dir_out, files[int(EightDays)].replace('weekly', 'monthly').replace('week','month'))
